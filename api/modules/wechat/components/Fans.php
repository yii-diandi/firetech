 ...
 * @Last Modified time: 2020-06-18 18:04:53
 */
...
class Fans extends BaseObject
{
    ...
    {
        $logPath =  Yii::getAlias("@runtime/wechat/" . date('ymd') . '/login.log');
        ...
        if ($isHave) {
            ...
            $res = $DdMember->signup($this->removeEmoji($nickname), 0, $password);
            // 更新openid
            ...
            $dataFans = [
                ...
                'openid'   => $users['openid'],
                'nickname' => $this->removeEmoji($users['nickname']),
                'groupid' => $res['member']['groupid'],
                'fans_info' => $users['openid'],
                'unionid' => $users['unionid'] ? $users['unionid'] : '',
                'gender' => $users['gender'],
